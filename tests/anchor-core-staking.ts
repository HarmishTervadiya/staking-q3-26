...
describe("anchor-core-staking", () => {
  ...
  const CLOCK_SYSVAR = new anchor.web3.PublicKey("SysvarC1ock11111111111111111111111111111111");
  let timeBaseMs: number | null = null;
  let traveledDays = 0;

  async function readClockMs(): Promise<number> {
    const acc = await provider.connection.getAccountInfo(CLOCK_SYSVAR, "confirmed");
  ...
  async function sendWithRetry(fn: () => Promise<string>, label: string, maxTries = 5): Promise<string> {
    for (let attempt = 1; attempt <= maxTries; attempt++) {
      try {
        ...
        console.log(`\n[${label}] retryable RPC error (attempt ${attempt}/${maxTries}): ${msg.slice(0, 200)}`);
  ...
  async function advanceTimeByDays(days: number): Promise<void> {
    ...
    traveledDays += days;
    // +120s buffer: chain keeps ticking in real seconds between travel and
    // the claim tx, so an exact N*86400 jump would floor to N-1 days.
    const target = timeBaseMs + traveledDays * MILLISECONDS_PER_DAY + 120000;
  ...
  it("Create a collection", async () => {
    ...
    const tx = await sendWithRetry(() => program.methods.createCollection(collectionName, collectionUri)
    .accountsPartial({
    ...
    .rpc({ commitment: "confirmed", skipPreflight: false }), "create-collection");
  ...
  it("Mint an NFT", async () => {
    ...
    const tx = await sendWithRetry(() => program.methods.mintAsset(nftName, nftUri)
    .accountsPartial({
    ...
    .signers([nftKeypair])
    .rpc({ commitment: "confirmed", skipPreflight: false }), "mint-asset");
    console.log("\nYour transaction signature", tx);
    console.log("NFT address", nftKeypair.publicKey.toBase58());
  });

  it("Initialize Config", async () => {
    const tx = await sendWithRetry(() => program.methods.initialize(REWARDS_BPS, FREEZE_PERIOD_IN_DAYS)
    .accountsPartial({
    ...
    .rpc({ commitment: "confirmed", skipPreflight: false }), "initialize");
    console.log("\nYour transaction signature", tx);
  ...
  it("Stake an NFT", async () => {
    const tx = await sendWithRetry(() => program.methods.stake()
    .accountsPartial({
      owner: provider.wallet.publicKey,
      updateAuthority,
      config,
    ...
    .rpc({ commitment: "confirmed", skipPreflight: false }), "stake");
    console.log("\nYour transaction signature", tx);
  ...
  it("Try to claim immediately after stake (no rewards yet)", async () => {
    try {
      const tx = await sendWithRetry(() => program.methods.claimRewards()
        .accountsPartial(claimAccounts())
        .rpc({ commitment: "confirmed", skipPreflight: false }), "claim-immediate");
      throw new Error(`Claim should have failed with no elapsed days, but succeeded with tx: ${tx}`);
    } catch (err) {
      if (err instanceof anchor.AnchorError && err.error.errorCode.code === "InvalidTimestamp") {
  ...
  it("Try to unstake an NFT before the freeze period ends", async () => {
    ...
    const userRewardsAta = getAssociatedTokenAddressSync(rewardsMint, provider.wallet.publicKey, false, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
    try {
      const tx = await sendWithRetry(() => program.methods.unstake()
      .accountsPartial({
        owner: provider.wallet.publicKey,
        updateAuthority,
        config,
      ...
      .rpc({ commitment: "confirmed", skipPreflight: false }), "unstake-early");
  ...
  it("Claim rewards while still staked", async () => {
    const tx = await sendWithRetry(() => program.methods.claimRewards()
      .accountsPartial(claimAccounts())
      .rpc({ commitment: "confirmed", skipPreflight: false }), "claim-1");
    console.log("\nYour transaction signature", tx);
  ...
  it("Try to claim twice in the same period (no double rewards)", async () => {
    try {
      const tx = await sendWithRetry(() => program.methods.claimRewards()
        .accountsPartial(claimAccounts())
        .rpc({ commitment: "confirmed", skipPreflight: false }), "claim-double");
      throw new Error(`Second claim should have failed with no elapsed days, but succeeded with tx: ${tx}`);
    } catch (err) {
      if (err instanceof anchor.AnchorError && err.error.errorCode.code === "InvalidTimestamp") {
        console.log("\nSecond claim failed as expected:", err.error.errorMessage);
      } else {
        throw err;
      }
    }
  ...
  it("Time travel 2 more days and claim again (only new days paid)", async () => {
    ...
    const tx = await sendWithRetry(() => program.methods.claimRewards()
      .accountsPartial(claimAccounts())
      .rpc({ commitment: "confirmed", skipPreflight: false }), "claim-2");
    console.log("\nYour transaction signature", tx);
    const balance = await getRewardsBalance();
  ...
  it("Time travel 2 more days and unstake (only remainder paid)", async () => {
    ...
    .rpc({ commitment: "confirmed", skipPreflight: false }), "unstake-final");
    console.log("\nYour transaction signature", tx);
    const balance = await getRewardsBalance();
  ...
  it("Restake after unstake works (freeze plugin update path)", async () => {
    ...
    .rpc({ commitment: "confirmed", skipPreflight: false }), "restake");
