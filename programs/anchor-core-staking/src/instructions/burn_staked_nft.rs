...
pub fn handler(ctx: Context<BurnStaked>) -> Result<()> {
    ...
    // And we Thaw the asset (update the FreezeDelegate Plugin to false) to it does not refuse to burn
    ...
    let amount = (staked_time as u64)
        ...
        .ok_or(ErrorCode::InvalidRewardsBps)?
        // Multiplying by 3x for massive reward
        .checked_mul(3 as u64)
        .ok_or(ErrorCode::InvalidRewardsBps)?;
